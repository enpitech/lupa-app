// Local type definition for Uppy locale
interface UppyLocale {
  strings: Record<string, string | Record<string, string>>;
  pluralize: (n: number) => number;
}

const he_IL: UppyLocale = {
  strings: {},
  pluralize(n: number) {
    if (n === 1) {
      return 0;
...
he_IL.strings = {
  ...
  dropPasteImportFiles: 'אפשר לגרור קבצים לפה או %{browse}',
